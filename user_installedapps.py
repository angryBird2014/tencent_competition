...
def getUserInstallAppAction():
    user_install_app = pd.read_csv(dirname + filename,chunksize=100)
    ...
    if os.path.exists(user_app_dump_path) and os.path.exists(app_user_dump_path):
        ...
        app_groupby_userID = user_install_app.groupby('userID',as_index=False)
        app_groupby_appID = user_install_app.groupby('appID',as_index=False)
        ...
        user_app_rate_mean = user_app_rate['user_app_rate'].mean()
        ...
        app_user_rate_mean = app_user_rate['app_user_rate'].mean()
        ...
        user_app_amount_dataframe = pd.merge(user_app_amount_dataframe,user_app_rate,how='left',on='userID')
        ...
        app_user_amount_dataframe = pd.merge(app_user_amount_dataframe,app_user_rate,how='left',on='appID')
