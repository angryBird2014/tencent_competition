...
def mergeAllData(fileName,merge_src_data_dump_path):
    ...
    del app_categories_df
    data_df = pd.merge(data_df,position_df,how='left',on='positionID')
    del position_df
    data_df = pd.merge(data_df,user_df,how='left',on='userID')
    del user_df

    data_df = pd.merge(data_df,user_install,how='left',on='userID')
    del user_install
    data_df = pd.merge(data_df,app_install,how='left',on='appID')
    del app_install

    data_df = pd.merge(data_df, user_active_amount, how='left', on='userID')
    del user_active_amount
    data_df = pd.merge(data_df, creativeID_active_amount, how='left', on='creativeID')
    del creativeID_active_amount

    data_df = pd.merge(data_df,user_appID_category,how='left',on='userID')
...
if __name__ == '__main__':
    ...
    mergeAllData(train_filename,merge_src_train_dump_path)
    mergeAllData(test_filename,merge_src_test_dump_path)